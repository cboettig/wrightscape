...
int main(void)
{
	...
	double mean[] = {1,2};
	double sigma[] = {4, 2, 2, 3};
	double out[2] = {0,0};

	gsl_vector_view mean_view = gsl_vector_view_array(mean, 2);
	gsl_matrix_view sigma_view = gsl_matrix_view_array(sigma, 2,2);
	gsl_vector_view out_view = gsl_vector_view_array(out, 2);
