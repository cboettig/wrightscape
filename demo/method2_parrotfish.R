...
require(geiger)
require(maticce)
require(wrightscape) # just for the format_data() function
require(pmc) # for the convert() function
...
trait_data[[1]] <- as.factor(trait_data[[1]])
# Get just the active trait # not sure if necessary
parrotfish_trait <- trait_data[c("Genus_species", "Reg", "gape.y")]
#names(parrotfish_trait)[3] <- "X" # not sure if necessary

paste("starting OUwie")

require(OUwie)
oumv <- OUwie(ape$phy,parrotfish_trait,model=c("OUMV"),root.station=TRUE, plot.resid=FALSE)
ouma <- OUwie(ape$phy,parrotfish_trait,model=c("OUMA"),root.station=TRUE, plot.resid=FALSE)
oumva <- OUwie(ape$phy,parrotfish_trait,model=c("OUMVA"),root.station=TRUE, plot.resid=FALSE)
...
#identical(owie1$Param.est, owie2$Param.est)
#
