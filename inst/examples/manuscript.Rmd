...
``` {r }
ggplot(subset(dat, model=="release" & parameter == "alpha" & value < 2)) + 
...
``` {r }
ggplot(subset(lr_dat, value>-1e4 & value < 1e4)) + geom_density(aes(value, fill=model), alpha=.5) + facet_wrap(~trait, scale="free_x") + geom_vline(data=LR, aes(xintercept=value))
````
